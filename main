...
XRAY_TUNING_STATE="$XRAY_TUNING_DIR/last_state"
XRAY_TUNING_REASON="$XRAY_TUNING_DIR/last_reason"
...
refresh_box_width() {
  ...
  # Mantém a janela estável: 78 colunas sempre que o terminal suportar.
  # Só reduz se o terminal for realmente menor, para não desmontar o layout.
  if (( cols >= 80 )); then
    BOX_WIDTH=78
    BOX_LOCKED_WIDTH=78
    return
  ...
  if (( safe < 40 )); then
    BOX_WIDTH=40
  elif (( safe < BOX_MIN_WIDTH )); then
    BOX_WIDTH=$safe
  else
    BOX_WIDTH=$safe
  fi
  BOX_LOCKED_WIDTH=$BOX_WIDTH
}
...
box_line_text() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    _box_emit_line "$line"
  done < <(printf '%s\n' "$content" | fold -s -w "$usable")
}

box_line_center() {
  local content="$1"
  local inner usable left right len
  inner="$(box_inner_width)"
  usable=$((inner - 2))
  content="${content//$'\n'/ }"
  content="${content:0:$usable}"
  len=${#content}
  left=$(( (usable - len) / 2 ))
  right=$(( usable - len - left ))
  printf '║ %*s%s%*s ║\n' "$left" '' "$content" "$right" ''
}
...
box_line_pair() {
  local left="$1"
  local right="$2"
  local inner usable left_clean right_clean left_len right_len gap left_width
  inner="$(box_inner_width)"
  usable=$((inner - 2))
  left_clean="${left//$'\n'/ }"
  right_clean="${right//$'\n'/ }"
  left_len=${#left_clean}
  right_len=${#right_clean}
  gap=3
  if (( left_len + right_len + gap <= usable )); then
    left_width=$((usable - gap - right_len))
    printf '║ %-*.*s %s ║\n' "$left_width" "$left_width" "$left_clean" "$right_clean"
  else
    box_line_text "$left_clean"
    [[ -n "$right_clean" ]] && box_line_text "$right_clean"
  fi
}
...
box_line_two_columns() {
  local left="$1"
  local right="$2"
  local inner usable col sep left_clean right_clean
  inner="$(box_inner_width)"
  usable=$((inner - 2))
  sep=' | '
  left_clean="${left//$'\n'/ }"
  ...
  if (( ${#left_clean} > col || ${#right_clean} > col )); then
    box_line_text "$left_clean"
    [[ -n "$right_clean" ]] && box_line_text "$right_clean"
    return
  fi

  printf '║ %-*.*s%s%-*.*s ║\n' "$col" "$col" "$left_clean" "$sep" "$col" "$col" "$right_clean"
}
...
box_action_screen() {
  ...
  header
  box_window_open "$title"
  if (($#)); then
    box_hr
    local line_text
    for line_text in "$@"; do
      box_line_text "$line_text"
    done
  fi
  box_bottom
  echo -e "${NC}"
...
header() {
  ...
  clear
  refresh_box_width
  local cpu_use cpu_threads mem_line mem_total mem_used storage os_short swap_line swap_used swap_total
  ...
  mem_total="${mem_line#*/}"
  swap_used="${swap_line%/*}"
  swap_total="${swap_line#*/}"
  storage="$(get_storage_display)"
  os_short="${OS_NAME} ${OS_VERSION}"

  echo -e "${MAGENTA}${BOLD}"
  box_top
  box_line_center "AWS"
  box_hr
  box_line_pair "Sistema: ${os_short}" "Espaço: ${storage}"
  box_line_pair "CPU: ${cpu_use}/${cpu_threads}" "RAM: ${mem_used}/${mem_total} (${swap_used}/${swap_total})"
  box_line_text "Ambiente: ${AWS_ENV_LABEL}"
  box_bottom
  echo -e "${NC}"
}
...
install_or_update_badvpn() {
  box_action_screen "INSTALAR/ATUALIZAR BADVPN" "Compila e publica o badvpn-udpgw no sistema"
...
restart_badvpn_current() {
  box_action_screen "REINICIAR BADVPN" "Reinicia o modo atual do serviço"
...
enable_badvpn_multiport() {
  box_action_screen "ATIVAR MULTIPORTA" "Ativa as portas padrão do BadVPN automaticamente"
...
restart_badvpn_multiport() {
  box_action_screen "REINICIAR MULTIPORTA" "Reinicia todas as instâncias multiporta do BadVPN"
...
remove_badvpn_multiport() {
  box_action_screen "REMOVER MULTIPORTA" "Volta o BadVPN para porta única"
...
toggle_badvpn_optimization() {
  box_action_screen "OTIMIZACAO BADVPN" "Ação automática: aplicar ou reverter tuning do BadVPN"
...
uninstall_badvpn_completely() {
  box_action_screen "REMOVER BADVPN" "Remove serviço, tuning e binários do BadVPN"
...
badvpn_menu_header() {
  header
  box_window_open "BADVPN"
  box_hr
  box_line_pair "Status: $(badvpn_parent_status)" "Modo: $(badvpn_mode_label)"
  box_line_pair "Portas: $(badvpn_ports_display)" "Otimização: $(badvpn_is_optimized && echo ATIVA || echo INATIVA)"
  box_bottom
  echo -e "${NC}"
}
...
badvpn_menu() {
  while true; do
    ...
    box_line_pair "Portas: $(badvpn_ports_display)" "Otimização: $(badvpn_is_optimized && echo ATIVA || echo INATIVA)"
    box_hr
    box_line_menu_plain "[01]" "Instalar/atualizar BadVPN"
    box_line_menu_plain "[02]" "Reiniciar BadVPN"
    box_line_menu_plain "[03]" "Ativar modo multiporta"
    box_line_menu_plain "[04]" "Reiniciar multiporta"
    box_line_menu_plain "[05]" "Remover multiporta"
    box_line_menu_plain "[06]" "Aplicar/reverter otimização do BadVPN"
    box_line_menu_plain "[07]" "Remover BadVPN"
    box_line_menu_plain "[00]" "Voltar"
    box_hr
    box_line_center "Digite a opção dentro do [ ]"
    box_window_close
    wait_key_menu
...
auto_prepare_system() {
  box_action_screen "ATUALIZAR SISTEMA" "Hostname fixo: ${CONFIG_HOSTNAME}" "Timezone fixa: ${CONFIG_TIMEZONE}"
...
change_root_password() {
  box_action_screen "SENHA ROOT" "A senha será alterada para o usuário root"
...
toggle_connection_improvements() {
  box_action_screen "MELHORAR CONEXAO" "Ação automática: aplicar ou reverter tuning de rede"
...
launch_external_menu() {
  ...
  box_action_screen "${title}" "Abrindo o menu oficial desta ferramenta..."
...
reinstaller_open_menu() {
  box_action_screen "FORMATAR SISTEMA" "Instala e abre o reinstalador oficial quando necessário"
...
dragoncore_open_menu() {
  box_action_screen "SCRIPT SSH" "Instala e abre o menu oficial do DragonCoreSSH-Beta"
...
xray_tuning_last_state() {
  [[ -f "$XRAY_TUNING_STATE" ]] && tr -d '\r\n' < "$XRAY_TUNING_STATE" || echo ""
}

xray_tuning_last_reason() {
  [[ -f "$XRAY_TUNING_REASON" ]] && cat "$XRAY_TUNING_REASON" || true
}
...
xray_apply_auto_tuning_core() {
  ...
  local variant variants rc test_ok=0
  cfg="$(xray_config_path)"
  tmp="$(mktemp)"
  ...
  variants=(full no_interval keepalive_only policy_only)
  for variant in "${variants[@]}"; do
    set +e
    meta_json="$(XRAY_PROFILE="$profile" XRAY_KEEPALIVE_IDLE="$keepalive_idle" XRAY_KEEPALIVE_INTERVAL="$keepalive_interval" XRAY_TCP_USER_TIMEOUT="$user_timeout" XRAY_CONN_IDLE="$conn_idle" XRAY_HANDSHAKE="$handshake" XRAY_UPLINK_ONLY="$uplink_only" XRAY_DOWNLINK_ONLY="$downlink_only" XRAY_CONGESTION="$congestion" XRAY_TRANSPORT="$transport" XRAY_VARIANT="$variant" python3 - "$cfg" "$tmp" <<'PYCODE'
import json, os, sys
...
variant = os.environ.get("XRAY_VARIANT", "full")
...
def apply_sockopt(ss):
    ...
    if variant in ("full", "no_interval"):
...
print(json.dumps({
    "profile": profile,
    "transport": transport,
    "congestion": congestion if variant != "policy_only" else "",
    "keepaliveIdle": keepalive_idle if variant != "policy_only" else 0,
    "keepaliveInterval": keepalive_interval if variant == "full" else 0,
    "tcpUserTimeout": user_timeout if variant in ("full", "no_interval") else 0,
    "connIdle": conn_idle,
    "handshake": handshake,
    "uplinkOnly": uplink_only,
    "downlinkOnly": downlink_only,
...
PYCODE
    ...
    if [[ "$rc" -eq 11 ]]; then
      printf '%s
' "no_target" > "$XRAY_TUNING_STATE"
      printf '%s
' "Nenhum inbound/outbound VLESS compatível encontrado." > "$XRAY_TUNING_REASON"
      rm -f "$tmp" "$XRAY_TUNING_MARKER" "$XRAY_TUNING_META"
      return 0
    elif [[ "$rc" -ne 0 ]]; then
      log "XRAY tuning python failed on variant=$variant rc=$rc"
      continue
    fi

    if xray_test_config_file "$tmp"; then
      test_ok=1
      break
    fi

    log "XRAY tuning config test failed on variant=$variant"
  done
...
' "$meta_json" > "$XRAY_TUNING_META"
    printf '%s
' "applied" > "$XRAY_TUNING_STATE"
    printf '%s
' "OK" > "$XRAY_TUNING_REASON"
    return 0
  fi

  cp -f "$XRAY_TUNING_BACKUP" "$cfg" 2>/dev/null || true
  rm -f "$tmp" "$XRAY_TUNING_MARKER" "$XRAY_TUNING_META"
  printf '%s
' "unsupported" > "$XRAY_TUNING_STATE"
  printf '%s
' "Seu XRAY rejeitou todos os perfis seguros do melhorador." > "$XRAY_TUNING_REASON"
  return 0
}
...
PYCODE
  ...
  cp -f "$tmp" "$cfg"
  rm -f "$tmp"
  rm -f "$XRAY_TUNING_MARKER" "$XRAY_TUNING_META" "$XRAY_TUNING_BACKUP" "$XRAY_TUNING_STATE" "$XRAY_TUNING_REASON"
}
...
limiter_menu() {
  while true; do
    ...
    header
    box_window_open "LIMITER SSH/XRAY"
    box_hr
    box_line_pair "Status: $(limiter_menu_status)" "XRAY: $(xray_support_enabled && echo PRONTO || echo INATIVO)"
    box_line_pair "Banco: ${LIMITER_DB}" "Usuários: $(limiter_user_count)"
    box_hr
    if limiter_installed || limiter_active; then
      box_line_menu_plain "[01]" "Desativar/desinstalar limiter"
    else
      box_line_menu_plain "[01]" "Instalar/ativar limiter"
    fi
    box_line_menu_plain "[02]" "Reiniciar limiter"
    box_line_menu_plain "[03]" "Sincronizar limites do DragonCore"
    box_line_menu_plain "[04]" "Ver status"
    box_line_menu_plain "[00]" "Voltar"
    box_hr
    box_line_center "Digite a opção dentro do [ ]"
    box_window_close
...
apply_xray_auto_tuning_only() {
  box_action_screen "MELHORADOR XRAY AUTOMATICO" "Detecta AWS/EC2 e ajusta o XRAY sem reduzir velocidade"
  ...
  case "$(xray_tuning_last_state)" in
    applied)
      ok "Melhorador XRAY automático ativado."
    ...
    no_target)
      warn "Nenhum alvo compatível encontrado no XRAY."
      [[ -n "$(xray_tuning_last_reason)" ]] && echo -e "${DIM}$(xray_tuning_last_reason)${NC}"
      ;;
    unsupported)
      warn "O XRAY deste servidor não aceitou os parâmetros seguros do melhorador."
      [[ -n "$(xray_tuning_last_reason)" ]] && echo -e "${DIM}$(xray_tuning_last_reason)${NC}"
      ;;
    *)
      if xray_auto_tuning_active; then
        ...
        warn "O melhorador não conseguiu concluir a aplicação."
      fi
      ;;
  esac
  pause
...
revert_xray_auto_tuning_only() {
  box_action_screen "REVERTER MELHORADOR XRAY" "Restaura o backup anterior do XRAY quando disponível"
...
updater_menu() {
  while true; do
    ...
    remote="$(fetch_update_remote_version)"
    header
    box_window_open "ATUALIZADOR"
    box_hr
    box_line_pair "Versão local: v${APP_VERSION}" "Versão remota: v${remote}"
    box_line_pair "Canal" "${UPDATE_CHANNEL}"
    box_hr
    box_line_menu_plain "[01]" "Atualizar menu agora"
    box_line_menu_plain "[00]" "Voltar"
    box_hr
    box_line_center "Defina UPDATE_SCRIPT_URL no topo do script para atualizar"
    box_window_close
    wait_key_menu
...
render_main_window() {
  local cpu_use cpu_threads mem_line mem_total mem_used storage os_short remote_version swap_line swap_used swap_total
  cpu_use="$(get_cpu_usage_percent)"
  cpu_threads="$(get_cpu_threads)"
  mem_line="$(free -h | awk '/^Mem:/ {print $3"/"$2}')"
  swap_line="$(free -h | awk '/^Swap:/ {print $3"/"$2}')"
  ...
  box_line_center "AWS"
  box_hr
  box_line_text "Sistema: ${os_short} | Espaço: ${storage}"
  box_line_text "CPU: ${cpu_use}/${cpu_threads} | RAM: ${mem_used}/${mem_total} (${swap_used}/${swap_total})"
  box_hr
  box_line_center "[A] Atualizar Script (v${APP_VERSION}/v${remote_version})"
  box_hr
  box_line_two_columns "[01] Atualizar Sistema" "[06] Melhorar Conexao"
  box_line_two_columns "[02] Senha Root" "[07] BadVPN"
  box_line_two_columns "[03] Formatar Sistema" "[08] Limiter SSH/XRAY"
  box_line_two_columns "[04] Script SSH" "[09] Melhorador XRAY"
  box_line_two_columns "[05] Checkuser DTunnel" ""
  box_hr
  box_line_center "Digite a opção dentro do [ ]"
  box_bottom
  echo -e "${NC}"
...
xray_tuning_menu() {
  while true; do
    header
    box_window_open "MELHORADOR XRAY"
    box_hr
    box_line_text "Status: $(xray_auto_tuning_status)"
    box_hr
    box_line_menu_plain "[01]" "Aplicar melhorador XRAY automático"
    box_line_menu_plain "[02]" "Reverter melhoria do XRAY"
    box_line_menu_plain "[00]" "Voltar"
    box_hr
    box_line_center "Detecta AWS/EC2 e escolhe o perfil automaticamente"
    box_window_close
    wait_key_menu
...
main_menu() {
  while true; do
    ...
    case "$MENU_KEY" in
      ...
      3) reinstaller_open_menu ;;
      4) dragoncore_open_menu ;;
      5) checkuser_open_menu ;;
      6) toggle_connection_improvements ;;
      7) badvpn_menu ;;
      8) limiter_menu ;;
      9) xray_tuning_menu ;;
      A) updater_menu ;;
